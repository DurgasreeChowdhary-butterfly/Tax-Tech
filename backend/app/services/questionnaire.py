...
def _get_filing_session_or_raise(db: Session, filing_session_id: uuid.UUID) -> FilingSession:
...
def _ensure_bound_version(db: Session, filing_session: FilingSession):
...
def _load_routing_inputs(db: Session, filing_session: FilingSession, version):
...
def get_current_question(db: Session, filing_session_id: uuid.UUID) -> Question | None:
    ...
    version = _ensure_bound_version(db, filing_session)
    questions, rules_by_question_id, answers_by_question_id = _load_routing_inputs(db, filing_session, version)
...
def submit_answer(db: Session, filing_session_id: uuid.UUID, question_id: uuid.UUID, value) -> QuestionAnswer:
    ...
    return questionnaire_repo.record_answer(
        db,
